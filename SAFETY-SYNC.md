...
| Ситуация | Действие | Не делать |
|----------|----------|-----------|
| **Сервер (прод) новее** — подтянуть контент к себе | `bash scripts/pull-prod.sh` (PROD → LOCAL) | — |
| **Локалка новее** — привести сервер к своему состоянию | Код: `git push` + `bash scripts/deploy-theme-to-vps.sh`. Контент/БД/медиа: выгрузить дамп и uploads с локалки и залить на сервер вручную (см. ниже) | **Не запускать pull-prod.sh** — он перезапишет локалку состоянием прода и вы потеряете локальные изменения. |

Если вы случайно запустили pull-prod при новее локальной копии: локальная БД восстанавливается из бэкапа `local_backup_YYYY-MM-DD_HHMM.sql` (создаётся перед каждым pull). Восстановление:  
`docker compose exec -T db mysql -u root -p"$DB_ROOT_PASSWORD" wordpress < local_backup_2026-02-26_0322.sql`  
(подставьте нужный файл бэкапа). Файлы в `wordpress-uploads/`, перезаписанные с прода, восстановить можно только из своего бэкапа (Time Machine и т.п.), если делали.

**Локалка новее — как выкатить контент на сервер:**  
1. Код темы уже выкатывается через `deploy-theme-to-vps.sh`.  
2. Чтобы обновить на проде БД и медиа из локалки: сделать дамп локальной БД, на сервере заменить им продовую БД, затем выполнить search-replace `https://localhost:8443` → URL прода; uploads скопировать с локалки на сервер (rsync в обратную сторону). Делать только когда уверены, что прод можно перезаписать.
...
- **Деплой на Timeweb Cloud:** сборка по **Dockerfile** в корне репозитория (последний коммит в GitHub). Один образ: Nginx + WordPress (PHP-FPM). Подробно: [docs/DEPLOY-TIMEWEB-CICD.md](docs/DEPLOY-TIMEWEB-CICD.md).
...
4. Докачивает файлы из `wordpress-uploads` с сервера (rsync). По умолчанию — **только недостающие** (`--ignore-existing`). Если изменения в ACF по картинкам не подтянулись (заменили изображения на проде, а локально остались старые), запустите **полную пересинхронизацию uploads**:

```bash
FULL_UPLOADS_SYNC=1 bash scripts/pull-prod.sh
```

Тогда все файлы из прода перезапишут локальные копии.
