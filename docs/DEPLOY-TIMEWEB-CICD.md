...
- **`nginx.prod.conf`** — конфиг Nginx внутри контейнера: порт 80, PHP через Unix-сокет `/tmp/php-fpm.sock`. Порт 9000 в контейнере отключён, чтобы платформа не направляла трафик на PHP-FPM напрямую (иначе возможен 403).
...
5. **Порт приложения: 80** — обязательно укажите в настройках приложения порт **80**. Если платформа обнаружит порт 9000 (из метаданных базового образа), не переключайте трафик на него: в контейнере слушает только Nginx на 80.
6. Деплой: при каждом push в `main` платформа пересобирает образ и перезапускает контейнер.

## Если снова 403

- В настройках приложения проверьте, что **порт приложения = 80** (не 9000). Трафик должен идти на Nginx.
- В образе порт 9000 отключён (PHP-FPM слушает только сокет), но в метаданных образа он может отображаться — игнорируйте, выберите 80.
- Логи приложения (не сборки): в панели Timeweb откройте **логи запущенного приложения** (Runtime/App logs). Там должны быть строки от Nginx и PHP при запросе к сайту. По ним можно увидеть причину 403 (например `Permission denied`, `connect() failed`).
