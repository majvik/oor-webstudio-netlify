...
// Локально (Docker Compose): loopback на localhost:8443 из контейнера идёт на ::1 и не доходит.
// Перенаправить на nginx:8443 + заголовок для авторизации.
// На проде nginx в том же контейнере — редирект не нужен и вызывает таймаут.
$loopback_host = isset( $_SERVER['HTTP_HOST'] ) ? $_SERVER['HTTP_HOST'] : '';
$is_local_env  = ( strpos( $loopback_host, 'localhost' ) !== false );
